...
// Segments an image into two groups of pixels, one above and the other below a value
...
// Segments a monochromatic image into two groups of pixels, one above and the other below a value
...
// Segments a colored image into two groups of pixels, one above and the other below a value
...
{
	long mean = 0;
	for (int j = 0; j < img.rows; j++) for (int i = 0; i < img.cols; i++) mean += img.at<uchar>(j, i);
...
{
	float mean = 0;
	for (int j = 0; j < img.rows; j++) for (int i = 2; i < img.cols * 3; i += 3) mean += img.at<float>(j, i);
	mean = mean / (img.rows * img.cols);
	return mean;
}
...
{
	bool color = true;
	if (img.type() == CV_32FC1 || img.type() == CV_8UC1)	color = false;
	Mat img_out(img.size(), img.type());
	for (int j = 0; j < img.rows; j++)
	{
		for (int i = 0; i < img.cols + color * img.cols * 2; i++)
...
{
	return (rgb) ? mul_brightness_rgb(img, c) : mul_brightness_y(img, c);
}
